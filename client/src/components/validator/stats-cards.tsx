...
import { TrendingUp, Clock, CheckCircle, Coins, Loader2 } from "lucide-react";
...
export function StatsCards() {
  ...
  const { validatorInfo, isLoading } = useValidatorGet(address as string) as {
  ...
  const initialStake = validatorInfo?.[0]
    ? Number(formatEther(validatorInfo[0]))
    : 0;
  const currentStake = validatorInfo?.[1]
    ? Number(formatEther(validatorInfo[1]))
    : 0;
  const repEarned = validatorInfo?.[2]
    ? Number(formatEther(validatorInfo[2]))
    : 0;
  const validationCount = validatorInfo?.[3] ? Number(validatorInfo[3]) : 0;
  const lastValidatedAt = validatorInfo?.[4]
    ? new Date(Number(validatorInfo[4]) * 1000)
    : null;
  ...
  const stats: StatCard[] = [
    {
      title: "Staked Amount",
      value: isLoading ? "Loading..." : `${currentStake.toFixed(4)} ETH`,
      subValue: isLoading
        ? "Fetching data..."
        : `Initial: ${initialStake.toFixed(4)} ETH`,
      icon: Coins,
      trend: isLoading
        ? "..."
    ...
    },
    {
      ...
      subValue: isLoading
        ...
        : "Validator Inactive",
      icon: Clock,
      trend: isLoading ? "..." : isActive ? "Active" : "Inactive",
      trendUp: isActive,
      color: isActive
    ...
    },
    {
      ...
      value: isLoading ? "Loading..." : validationCount.toString(),
      subValue: "All Time",
      icon: CheckCircle,
      trend: isLoading ? "..." : validationCount > 0 ? "Active" : "No Activity",
    ...
    },
    {
      ...
      value: isLoading ? "Loading..." : `${repEarned.toFixed(4)} REP`,
      subValue: "Total Earnings",
      icon: TrendingUp,
      trend: isLoading ? "..." : repEarned > 0 ? "+🔼" : "No Earnings",
      trendUp: repEarned > 0,
      color: "text-green-600 dark:text-green-400",
      bgColor: "bg-green-500/10",
      borderColor: "border-green-500/20",
